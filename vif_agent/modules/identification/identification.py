...
from vif_agent.prompt import DETECTION_PROMPT
...
class LLMIdentificationModule(IdentificationModule):
...
class BoxIdentificationModule(LLMIdentificationModule):
    ...
    def identify(self, code: str, features: list[str], base_image: Image.Image):
        ...
        if not id_match:
            logger.warning(
                f"Feature identification failed, using un-identified code, unparseable response {response.choices[0].message.content}"
            )
